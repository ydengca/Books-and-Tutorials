...
--****************** [DWAdventureWorksLT2012Lab02] *********************--
-- This file will drop and create the DWAdventureWorksLT2012Lab02
-- database, with all its objects. 
--********************************************************************--

USE [master];
If Exists (Select Name from SysDatabases Where Name = 'DWAdventureWorksLT2012Lab02')
  Begin
   Alter database DWAdventureWorksLT2012Lab02 set single_user with rollback immediate;
   Drop database DWAdventureWorksLT2012Lab02;
  End
go
CREATE DATABASE DWAdventureWorksLT2012Lab02;
go
...
While @DateInProcess <= @EndDate
	...
	--Add a row into the date dimension table for this date
		Insert Into [DWAdventureWorksLT2012Lab02].[dbo].[DimDates] 
		( [CalendarDateKey]
		, [CalendarDateName]
		, [CalendarYearMonthID]
...
AS
	...
	JOIN [DWAdventureWorksLT2012Lab02].[dbo].[DimCustomers] as T3
		ON T2.[CustomerID] = T3.[CustomerID]
	JOIN [DWAdventureWorksLT2012Lab02].[dbo].[DimProducts] as T4
		ON T4.[ProductID] = T1.[ProductID]
	JOIN [DWAdventureWorksLT2012Lab02].[dbo].[DimDates] as T5
		ON Cast(T5.CalendarDate as Date) = Cast(T2.[OrderDate] as Date)
	JOIN [DWAdventureWorksLT2012Lab02].[dbo].[DimDates] as T6
...
Begin -- Procedure Code
 ...
 Begin Try 
  Begin Transaction; 
  -- ETL Code  -------------------------------------------------------------------
  		INSERT INTO [DWAdventureWorksLT2012Lab02].[dbo].[DimCustomers]
		( [CustomerID]
		, [ContactFullName]
		, [CompanyName]
		)
		SELECT 
			...
			, [CompanyName]
			, [ContactFullName]
		FROM  [DWAdventureWorksLT2012Lab02].[dbo].[vETLDimCustomersData] 
  -- ETL Code  -------------------------------------------------------------------
  Commit Transaction;
  Set @RC = 100; -- Success
 End Try
...
Begin -- Procedure Code
 ...
 Begin Try 
  Begin Transaction; 
  -- ETL Code  -------------------------------------------------------------------
		INSERT INTO [DWAdventureWorksLT2012Lab02].[dbo].[DimProducts]
		( [ProductID]
		, [ProductName]
		, [ProductColor]
		, [ProductListPrice]
		...
		SELECT 
			...
			,[ProductCategoryID]
			,[ProductCategoryName]
		FROM  [DWAdventureWorksLT2012Lab02].[dbo].[vETLDimProductsData]
  -- ETL Code  -------------------------------------------------------------------
  Commit Transaction;
  Set @RC = 100; -- Success
 End Try
...
Begin -- Procedure Code
 ...
 Begin Try 
  Begin Transaction; 
  -- ETL Code  -------------------------------------------------------------------
		INSERT INTO [DWAdventureWorksLT2012Lab02].[dbo].[FactSales]
		( [SalesOrderID]
		, [SalesOrderDetailID]
		, [CustomerKey]
		, [ProductKey]
		...
		SELECT 
			...
			, [UnitPrice]
			, [UnitPriceDiscount]
			FROM [DWAdventureWorksLT2012Lab02].[dbo].[vETLFactSalesData] 
